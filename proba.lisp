...
(defun dodajPotez(potez s)
	...
	(format t "graf ~a ~%" grafSusedaX)
	(format t "lista krajnjih ~a~%" listaKrajnjihCvorova)
	(format t "lista poteza ~a~%" listaPotezaX)
	(nadji listaKrajnjihCvorova listaPotezaX)
	(format t "~a~%" krajnji)
	(ispitaj)
	(listaKrajnjih)
	(format t "obradjeni~a~%" obradjeni)
)
(defun ispitaj()
	(if(>= (length krajnji) 2) (ispitajMost) nil)
)
(defun ispitajMost()
	;(format t "most~%" )
	;(format t "lista poredjenja ~a~%" (uporediDva krajnji grafSusedaX))
	(format t "put ~a~%" (nadji-put grafSusedaX listaPotezaX (cadr krajnji) obradjeni))
	
)
(defun nadji-put (graf l cilj cvorovi)
(format t "graf ~a~%" graf )
(format t "l ~a~%" l )
(format t "cilj ~a~%" cilj )
(format t "lista obra ~a~%" cvorovi )
   ...
   (format t "cilj ~a~%" cvorovi1 )
   (format t "lista obra ~a~%" potomci1 )
(format t "lista obra ~a~%" l1 )
)
...
(defun uporediDva(potencijalniSus listaPote)
	(if (null (car listaPote))
		(uporediDva (cdr potencijalniSus) grafSusedaX)
			;else
			(if (null (car potencijalniSus)) '()
				;else
				(if (equal (caar potencijalniSus) (caaar listaPote)) 
					(if (equal (cadr potencijalniSus) (caadr listaPote))
					;(append (list(car potencijalniSus)) (nadji1 (cdr potencijalniSus) grafSusedaX))
					(and (setq obradjeni (car potencijalniSus)) (nadji1 (cadar listaPote) grafSusedaX))
					;else
					(uporediDva potencijalniSus (cdr listaPote))
					)
					;else
					(uporediDva potencijalniSus (cdr listaPote))
				)
...
(defun nadji1(potencijalniSus listaPote)
(format t "cvor 1 ~a~%" potencijalniSus )
(format t "cvor 2 ~a~%" listaPote )
	(if (null (car listaPote))
		(nadji1 (cdr potencijalniSus) grafSusedaX)
			;else
			(if (null (car potencijalniSus)) '()
				;else
				(if (equal (caar potencijalniSus) (caaar listaPote)) 
					(if (equal (cadr potencijalniSus) (caadr listaPote))
					(if (uporediDvaCvora (car obradjeni) (cadr listaPote))
					;(equal (car obradjeni) (cadr(caar listaPote)))
						(append (cddar listaPote))
						;else
						;(append (caadar listaPote) (nadji1 (cdr potencijalniSus) grafSusedaX))
						(append (cadar listaPote))
					)
					;else
					(nadji1 potencijalniSus (cdr listaPote))
					)
					;else
					(nadji1 potencijalniSus (cdr listaPote))
				)
...
(defun nadji(potencijalniSus listaPote)
	(if (null (car listaPote))
		(nadji (cdr potencijalniSus) listaPotezaX)
			;else
			(if (null (car potencijalniSus)) '()
				;else
				(if (equal (caar potencijalniSus) (caar listaPote)) 
					(if (equal (cdar potencijalniSus) (cdar listaPote))(setq krajnji(append (list(car potencijalniSus)) (nadji (cdr potencijalniSus) listaPotezaX)))
					;else
					(nadji potencijalniSus (cdr listaPote))
					)
					;else
					(nadji potencijalniSus (cdr listaPote))
				)
